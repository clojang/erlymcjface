...
/**
 * Utility class for converting exceptions to Result types. Provides a safe way to handle operations
...
public final class Try {
  ...
  /**
   * Execute a supplier with a specific exception type. If an unexpected exception type occurs,
   * returns a Result with Exception instead of the specific type.
   */
  public static <T, E extends Exception> Result<T, Exception> ofType(
      Class<E> exceptionType, Supplier<T> supplier) {
    try {
      return Result.ok(supplier.get());
    } catch (Exception e) {
  ...
  public static <T, E> Result<T, E> withMapping(
      Supplier<T> supplier, Function<Exception, E> errorMapper) {
    try {
      return Result.ok(supplier.get());
    } catch (Exception e) {
